...
import * as os from './m_os';
import array from './array';
import parse from './parse';
import types from './types';
import Schema from './schema';
import checkOffset from './check-offset';
import checkTargetBuffer from './check-target-buffer';
...
function structWrite(
  ...
  const type = types.get(typeName);
  const { buffer, offset } = checkTargetBuffer(
    targetBuffer,
    targetOffset,
    type.byteLength,
  );
...
struct.writeArray = function writeArray(
  typeName,
  data,
  targetBuffer,
  targetOffset,
) {
  return structWriteArray(
    os.endianness(),
    typeName,
    data,
    targetBuffer,
...
struct.writeArrayLE = function writeArrayLE(
  typeName,
  data,
  targetBuffer,
  targetOffset,
...
struct.writeArrayBE = function writeArrayBE(
  typeName,
  data,
  targetBuffer,
  targetOffset,
