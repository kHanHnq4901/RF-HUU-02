const types = new Map();
{
  const readIntLE = (bytes) => (buffer, offset) =>
    buffer.readIntLE(offset, bytes, true);
  const readIntBE = (bytes) => (buffer, offset) =>
    buffer.readIntBE(offset, bytes, true);
  const writeIntLE = (bytes) => (value, buffer, offset) =>
    buffer.writeIntLE(value, offset, bytes, true);
  const writeIntBE = (bytes) => (value, buffer, offset) =>
  ...
  const writeBigInt64BE = () => (value, buffer, offset) =>
    buffer.writeBigInt64BE(value, offset);

  types.set('int8', {
    byteLength: 1,
    readLE: readIntLE(1),
    writeLE: writeIntLE(1),
    readBE: readIntBE(1),
    writeBE: writeIntBE(1),
  });
  types.set('int16', {
    byteLength: 2,
    readLE: readIntLE(2),
    writeLE: writeIntLE(2),
    readBE: readIntBE(2),
    writeBE: writeIntBE(2),
  });
  types.set('int32', {
    byteLength: 4,
    readLE: readIntLE(4),
    writeLE: writeIntLE(4),
    readBE: readIntBE(4),
    writeBE: writeIntBE(4),
  });
  types.set('int64', {
    byteLength: 8,
    readLE: readBigInt64LE(),
    writeLE: writeBigInt64LE(),
    readBE: readBigInt64BE(),
    writeBE: writeBigInt64BE(),
  });
}
{
  const readUIntLE = (bytes) => (buffer, offset) =>
    buffer.readUIntLE(offset, bytes, true);
  const readUIntBE = (bytes) => (buffer, offset) =>
    buffer.readUIntBE(offset, bytes, true);
  const writeUIntLE = (bytes) => (value, buffer, offset) =>
    buffer.writeUIntLE(value, offset, bytes, true);
  const writeUIntBE = (bytes) => (value, buffer, offset) =>
    buffer.writeUIntBE(value, offset, bytes, true);

  const readBigUInt64LE = () => (buffer, offset) =>
    buffer.readBigUInt64LE(offset);
  const readBigUInt64BE = () => (buffer, offset) =>
    buffer.readBigUInt64BE(offset);
  const writeBigUInt64LE = () => (value, buffer, offset) =>
    buffer.writeBigUInt64LE(value, offset);
  const writeBigUInt64BE = () => (value, buffer, offset) =>
    buffer.writeBigUInt64BE(value, offset);

  types.set('uint8', {
    byteLength: 1,
    readLE: readUIntLE(1),
    writeLE: writeUIntLE(1),
    readBE: readUIntBE(1),
    writeBE: writeUIntBE(1),
  });
  types.set('uint16', {
    byteLength: 2,
    readLE: readUIntLE(2),
    writeLE: writeUIntLE(2),
    readBE: readUIntBE(2),
    writeBE: writeUIntBE(2),
  });
  types.set('uint32', {
    byteLength: 4,
    readLE: readUIntLE(4),
    writeLE: writeUIntLE(4),
    readBE: readUIntBE(4),
    writeBE: writeUIntBE(4),
  });
  types.set('uint64', {
    byteLength: 8,
    readLE: readBigUInt64LE(),
    writeLE: writeBigUInt64LE(),
    readBE: readBigUInt64BE(),
    writeBE: writeBigUInt64BE(),
  });
}

{
  ...
  const writeFloatLE = (value, buffer, offset) =>
    buffer.writeFloatLE(value, offset, true);
  const writeFloatBE = (value, buffer, offset) =>
...

{
  ...
  const writeDoubleLE = (value, buffer, offset) =>
    buffer.writeDoubleLE(value, offset, true);
  const writeDoubleBE = (value, buffer, offset) =>
...

{
  ...
  const writeBool = (value, buffer, offset) =>
